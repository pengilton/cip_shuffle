    ...
    if (my_file.is_open()) {
        ...
        for (std::size_t i = benchmark.min_exp; i <= benchmark.max_exp; i++) {
            ...
            while (true) {
                auto start = std::chrono::steady_clock::now();
                for (std::size_t i = 0; i < benchmark.total_runs; i++) {
                    std::shuffle(view.begin(), view.end(), generator);
                }
                auto end = std::chrono::steady_clock::now();
    ...
    if (my_file.is_open()) {
        ...
        for (std::size_t i = benchmark.min_exp; i <= benchmark.max_exp; i++) {
            ...
            while (true) {
                auto start = std::chrono::steady_clock::now();
                for (std::size_t i = 0; i < benchmark.total_runs; i++) {
                    fisher_yates_shuffle(view, generator);
                }
                auto end = std::chrono::steady_clock::now();
    ...
    if (my_file.is_open()) {
        ...
        for (std::size_t i = benchmark.min_exp; i <= benchmark.max_exp; i++) {
            ...
            while (true) {
                auto start = std::chrono::steady_clock::now();
                for (std::size_t i = 0; i < benchmark.total_runs; i++) {
                    fisher_yates_shuffle_32(view, generator);
                }
                auto end = std::chrono::steady_clock::now();
    ...
    if (my_file.is_open()) {
        ...
        for (std::size_t i = benchmark.min_exp; i <= benchmark.max_exp; i++) {
            ...
            while (true) {
                auto start = std::chrono::steady_clock::now();
                for (std::size_t i = 0; i < benchmark.total_runs; i++) {
                    fisher_yates_shuffle_64(view, generator);
                }
                auto end = std::chrono::steady_clock::now();
...
// Note that his one will either use buffered_fisher_yates_32 or buffered_fisher_yates_64. It depnds on the size of the data. 
// In most cases it should use the 32-bit one becasue we won't use fisher-yates for such large data.
    ...
    if (my_file.is_open()) {
        ...
        for (std::size_t i = benchmark.min_exp; i <= benchmark.max_exp; i++) {
            ...
            while (true) {
                auto start = std::chrono::steady_clock::now();
                for (std::size_t i = 0; i < benchmark.total_runs; i++) {
                    buffered_fisher_yates_shuffle_64(view, generator);
                }
                auto end = std::chrono::steady_clock::now();
